...
# NEW: Directory for persistent Telegram sessions
...
class EnhancedRiskManager:
    ...
    def check_margin_level(self, balance_info, min_margin_level=1.5):
        ...
        try:
            ...
            if margin_level < min_margin_level:
                alert_msg = f"⚠️ LOW MARGIN LEVEL: {
                    margin_level:.2f} (Min: {min_margin_level})"
    ...
    def can_trade(self, daily_limit, weekly_limit,
                  max_consecutive_losses, balance_info=None, min_margin_level=1.5):
        ...
        if balance_info:
            margin_safe, margin_msg = self.check_margin_level(
                balance_info, min_margin_level)
            if not margin_safe:
        ...
        if self.tracking.get('cooling_until'):
            cooling_until = datetime.fromisoformat(
                self.tracking['cooling_until'])
            if datetime.now() < cooling_until:
                remaining = (
                    cooling_until - datetime.now()).total_seconds() / 60
                ws_logger.log(
                    f"❄️ Cooling period aktywny jeszcze przez {
                        remaining:.0f} minut", 'warning')
        ...
        if abs(daily_loss) >= daily_limit:
            ws_logger.log(
                f"🛑 Osiągnięto dzienny limit strat: ${
                    abs(daily_loss):.2f} / ${
                    daily_limit:.2f}", 'error')
        ...
        if abs(weekly_loss) >= weekly_limit:
            ws_logger.log(
                f"🛑 Osiągnięto tygodniowy limit strat: ${
                    abs(weekly_loss):.2f} / ${
                    weekly_limit:.2f}", 'error')
        ...
        if self.tracking['consecutive_losses'] >= max_consecutive_losses:
            ws_logger.log(
                f"🛑 Zbyt wiele strat z rzędu: {
                    self.tracking['consecutive_losses']}",
                'error')
    ...
    def record_trade(self, profit_loss, symbol):
        ...
        # Log status
        ws_logger.log(
            f"📊 Trade recorded: {symbol} P/L: ${profit_loss:.2f}", 'info')
        ws_logger.log(
            f"📈 Daily P/L: ${self.tracking['daily_losses'][today]:.2f}", 'info')
        ws_logger.log(
            f"📅 Weekly P/L: ${self.tracking['weekly_losses'][week]:.2f}", 'info')
    ...
    def get_stats(self):
        ...
        stats = {
            ...
            # Last 5 alerts
            'recent_margin_alerts': self.tracking.get('margin_alerts', [])[-5:]
        }
...
class TelegramForwarder:
    ...
    def load_forwarder_config(self):
        ...
        try:
            ...
            if config.has_section('Forwarder'):
                ...
                self.api_hash = config.get(
                    'Forwarder', 'api_hash', fallback='')
                self.phone_number = config.get(
                    'Forwarder', 'phone_number', fallback='')
                self.target_chat_id = config.get(
                    'Forwarder', 'target_chat_id', fallback='')
                self.forward_all_messages = config.get(
                    'Forwarder', 'forward_all_messages', fallback='False').lower() == 'true'

                # Load monitored channels as JSON
                channels_json = config.get(
                    'Forwarder', 'monitored_channels', fallback='[]')
                try:
            ...
            ws_logger.log(
                f"📱 Forwarder config loaded: API ID: {
                    bool(
                        self.api_id)}, Session exists: {
    ...
    def save_forwarder_config(self):
        ...
        try:
            ...
            config.set(
                'Forwarder', 'api_id', str(
                    self.api_id) if self.api_id else '')
            config.set(
                'Forwarder', 'api_hash', str(
                    self.api_hash) if self.api_hash else '')
            config.set('Forwarder', 'phone_number', str(
                self.phone_number) if self.phone_number else '')
            config.set('Forwarder', 'target_chat_id', str(
                self.target_chat_id) if self.target_chat_id else '')
            config.set(
                'Forwarder', 'forward_all_messages', str(
    ...
    def initialize_telethon_client(self):
        ...
        try:
            ...
            with self._client_lock:
                ...
                ws_logger.log(
                    "✅ Telethon client initialized successfully",
                    'success')
                return True
    ...
    async def connect_and_get_channels(self):
        ...
        try:
            ...
            try:
                ...
                if session_exists:
                    ws_logger.log(
                        "📂 Found existing session, attempting connection...", 'info')
                    ...
                    try:
                        ...
                        if await self.telethon_client.is_user_authorized():
                            ws_logger.log(
                                "✅ Connected using existing session! No 2FA needed.", 'success')
                            self._session_authorized = True
                        else:
                            ws_logger.log(
                                "⚠️ Session expired, re-authorization required", 'warning')
                        ...
                        ws_logger.log(
                            f"⚠️ Session error: {e}, creating new session...", 'warning')
                        await self.telethon_client.start(
                            phone=self.phone_number,
                            code_callback=self._auth_code_callback,
                            password=self._auth_password_callback
                    ...
                    ws_logger.log(
                        "📱 No existing session, authorization required", 'info')
                ...
                ws_logger.log(
                    "❌ Verification code expired. Try again.", 'error')
                self.channels_cache['is_loading'] = False
                socketio.emit('forwarder_channels_loading', {'loading': False})
                return False
                ...
                ws_logger.log(
                    f"❌ Too many attempts. Wait {
                        e.seconds} seconds.", 'error')
                self.channels_cache['is_loading'] = False
                socketio.emit('forwarder_channels_loading', {'loading': False})
                return False
            ...
            ws_logger.log(
                f"✅ Logged in as: {
                    self.console_session_info['first_name']} (@{
                    self.console_session_info['username']})")
        ...
        try:
            ...
            try:
                success = loop.run_until_complete(
                    self.connect_and_get_channels())
                if success:
                    ws_logger.log(
                        f"✅ Successfully fetched {len(self.available_channels)} channels", 'success')
    ...
    def start_forwarder(self):
        ...
        try:
            ...
            if not TELETHON_AVAILABLE:
                ws_logger.log(
                    "❌ Telethon not installed - forwarder cannot run", 'error')
                return False

            # Validate configuration
            if not self.api_id or not self.api_hash:
                ws_logger.log(
                    "❌ Missing API ID or API Hash - configure Telethon data first", 'error')
                return False

            if not self.phone_number:
                ws_logger.log(
                    "❌ Missing phone number - enter phone number", 'error')
                return False
            ...
            if not self.target_chat_id:
                ws_logger.log(
                    "⚠️ No target Chat ID - using bot's Chat ID",
                    'warning')
                self.target_chat_id = self.bot.telegram_chat_id

            if not self.target_chat_id:
                ws_logger.log(
                    "❌ No target Chat ID for forwarding messages", 'error')
                return False
            ...
            ws_logger.log(
                f"✅ Forwarder started - forwarding to Chat ID: {self.target_chat_id}", 'success')
            return True
        ...
        try:
            ...
            if worker_session_exists:
                ...
                if not await forwarder_client.is_user_authorized():
                    ws_logger.log(
                        "⚠️ Session expired, re-authorizing", 'warning')
                    ...
                    ws_logger.log(
                        "✅ Forwarder connected with existing session", 'success')
            else:
            ...
            for ch in self.monitored_channels:
                try:
                    ...
                    ws_logger.log(
                        f"📺 Channel {
                            ch['name']}: starting message_id = {last_message_id}")
                except Exception as e:
                    ws_logger.log(
                        f"❌ Error initializing channel {
                            ch.get(
                                'name',
                                'Unknown')}: {e}",
                        'error')
            ...
            if not monitored_channels_data:
                ws_logger.log(
                    "❌ No channels available for monitoring", 'error')
                await forwarder_client.disconnect()
                return

            ws_logger.log(
                f"🔍 POLLING: Monitoring {
                    len(monitored_channels_data)} channels...")
            ...
            while self.forwarder_running:
                try:
                    ...
                    for channel_id, channel_data in monitored_channels_data.items():
                        try:
                            ...
                            if new_messages and len(new_messages) > 0:
                                actual_new = len(
                                    [m for m in new_messages if m.id > last_message_id])
                                if actual_new > 0:
                                    ws_logger.log(
                                        f"📨 {actual_new} new messages from {channel_name}")

                        except Exception as e:
                            ws_logger.log(
                                f"❌ Error checking channel {
                                    channel_data.get(
                                        'name', 'Unknown')}: {e}", 'error')
                            continue
                    ...
                    ws_logger.log(
                        f"❌ Error in main polling loop: {e}", 'error')
    ...
    async def process_message(self, message, channel_name, client=None):
        ...
        try:
            ...
            ws_logger.log(
                f"📨 New message from {channel_name}: {message_text[:50]}...")
            ...
            if signal:
                ...
                ws_logger.log(
                    f"✅ Signal recognized from {channel_name}: {
                        signal['symbol']} {
                ...
                if self.bot.auto_execute_signals:
                    try:
                        if self.bot.bybit_client:
                            ...
                            ws_logger.log(
                                f"🤖 Auto-trade from {channel_name}: {result}")
                        ...
                        error_msg = f"❌ Auto-trade error from {channel_name}: {
                            str(e)}"
                    ...
                    ws_logger.log(
                        f"ℹ️ Signal recognized from {channel_name} (auto-trade disabled)")
                ...
                ws_logger.log(
                    f"ℹ️ Message from {channel_name} is not a trading signal (skipping)")

        except Exception as e:
            ws_logger.log(
                f"❌ Error processing message from {channel_name}: {e}",
                'error')
    ...
    async def send_to_target_chat(self, message):
        ...
        try:
            if not self.bot.telegram_bot or not self.target_chat_id:
                ws_logger.log(
                    "⚠️ No Telegram bot or target Chat ID",
                    'warning')
                return False
...
class PositionManager:
    ...
    def add_position(self, symbol, signal, order_id):
        ...
        try:
            ...
            ws_logger.log(
                f"✅ Position {symbol} added to monitoring",
                'success')
            ws_logger.log(
                f"💡 Breakeven will be activated after TP{
                    self.bot.breakeven_after_target}", 'info')
            ...
            ws_logger.log(
                f"❌ Error adding position to monitoring: {e}",
                'error')
            return False
    ...
    def check_target_reached_by_price(self, symbol):
        ...
        try:
            ...
            if not current_price:
                ws_logger.log(
                    f"⚠️ Cannot get current price for {symbol}",
                    'warning')
                return False
            ...
            for target_num, target_price in sorted(targets.items()):
                ...
                if position_type == "long":
                    ...
                    if current_price >= target_price:
                        target_reached = True
                        ws_logger.log(
                            f"🎯 LONG TP{target_num} REACHED! Price {current_price} >= Target {target_price}",
                            'success')
                    ...
                    if current_price <= target_price:
                        target_reached = True
                        ws_logger.log(
                            f"🎯 SHORT TP{target_num} REACHED! Price {current_price} <= Target {target_price}",
                            'success')
                ...
                if target_reached:
                    ...
                    ws_logger.log(
                        f"✅ Target {target_num} added to reached for {symbol}", 'success')
                else:
                    if position_type == "long":
                        distance = (
                            (target_price - current_price) / current_price) * 100
                        ws_logger.log(
                            f"⏳ TP{target_num}: Need +{distance:.2f}% to {target_price}")
                    else:
                        distance = (
                            (current_price - target_price) / current_price) * 100
                        ws_logger.log(
                            f"⏳ TP{target_num}: Need -{distance:.2f}% to {target_price}")
            ...
            if newly_filled:
                ws_logger.log(
                    f"🎉 NEW TARGETS REACHED for {symbol}: {newly_filled}",
                    'success')
                ...
                if breakeven_target in newly_filled and not position[
                        'sl_moved_to_breakeven'] and self.bot.auto_breakeven:
                    ws_logger.log(
                        f"💡 Target {breakeven_target} reached for {symbol} - initiating breakeven",
                        'info')
                    breakeven_success = self.move_sl_to_breakeven(symbol)
                    if breakeven_success:
                        ws_logger.log(
                            f"🎉 BREAKEVEN ACTIVATED for {symbol} after TP{breakeven_target}!", 'success')
                    else:
                        ws_logger.log(
                            f"❌ Error activating breakeven for {symbol}", 'error')
            ...
            ws_logger.log(
                f"❌ Error checking targets for {symbol}: {e}",
                'error')
            return False
    ...
    def move_sl_to_breakeven(self, symbol):
        ...
        try:
            if symbol not in self.active_positions:
                ws_logger.log(
                    f"❌ Position {symbol} not in monitoring", 'error')
                return False
            ...
            if position['sl_order_id']:
                ws_logger.log(
                    f"🗑️ Canceling old SL: {
                        position['sl_order_id']}")
                cancel_success = self.cancel_sl_order(
                    symbol, position['sl_order_id'])
                if cancel_success:
                    ws_logger.log(f"✅ Old SL canceled for {symbol}", 'success')
                else:
                    ws_logger.log(
                        f"⚠️ Failed to cancel old SL for {symbol}, continuing...", 'warning')
            ...
            if success:
                ...
                ws_logger.log(
                    f"🎉 SUCCESS! SL moved to breakeven for {symbol} at level {entry_price}",
                    'success')
                ...
                if hasattr(
                        self.bot, 'telegram_bot') and self.bot.telegram_bot and self.bot.telegram_chat_id:
                    try:
                        message = (
                        ...
                        ws_logger.log(
                            f"📱 Breakeven notification sent for {symbol}", 'success')
                    except Exception as e:
                        ws_logger.log(
                            f"⚠️ Failed to send breakeven notification: {e}", 'warning')

                return True
            else:
                ws_logger.log(
                    f"❌ ERROR! Failed to set new SL at breakeven for {symbol}", 'error')
                return False

        except Exception as e:
            ws_logger.log(
                f"❌ Error moving SL to breakeven for {symbol}: {e}",
                'error')
            return False
    ...
    def setup_tp_sl_orders(self, symbol):
        ...
        try:
            ...
            if current_size <= 0:
                ws_logger.log(
                    f"❌ No active position for {symbol} - cannot set TP/SL", 'error')
                return False
            ...
            if targets:
                ...
                for target_num, target_price in sorted_targets:
                    total_orders += 1
                    try:
                        success = self.set_take_profit(
                            symbol, target_num, target_price)
                        if success:
                            success_count += 1
                            ws_logger.log(
                                f"✅ TP{target_num} set for {symbol} @ {target_price}", 'success')
                        else:
                            ws_logger.log(
                                f"❌ Error setting TP{target_num} for {symbol}", 'error')
                    except Exception as e:
                        ws_logger.log(
                            f"❌ Exception TP{target_num} for {symbol}: {e}", 'error')
            ...
            if stop_loss:
                ...
                try:
                    ...
                    if success:
                        success_count += 1
                        ws_logger.log(
                            f"✅ SL set for {symbol} @ {stop_loss}", 'success')
                    else:
                        ws_logger.log(
                            f"❌ Error setting SL for {symbol}", 'error')
                except Exception as e:
            ...
            if total_orders > 0:
                ...
                ws_logger.log(
                    f"📊 TP/SL for {symbol}: {success_count}/{total_orders} ({success_rate:.0f}%)")
                return success_count > 0
            else:
                ws_logger.log(
                    f"⚠️ No targets and SL to set for {symbol}",
                    'warning')
                return True

        except Exception as e:
            ws_logger.log(
                f"❌ Error setup_tp_sl_orders for {symbol}: {e}",
                'error')
            return False
        ...
        try:
            ...
            if position_type == "long":
                if target_price <= entry_price:
                    ...
                    ws_logger.log(error_msg, 'error')
                    ws_logger.log(
                        "💡 For LONG: Take Profit must be ABOVE entry price!", 'warning')
                    return False
            elif position_type == "short":
                if target_price >= entry_price:
                    ...
                    ws_logger.log(error_msg, 'error')
                    ws_logger.log(
                        "💡 For SHORT: Take Profit must be BELOW entry price!", 'warning')
                    return False

            ws_logger.log(
                f"✅ TP{target_num} validation PASSED for {
                    position_type.upper()} {symbol}", 'success')
            ...
            if not symbol_info:
                ws_logger.log(
                    f"❌ Cannot get symbol info for {symbol}", 'error')
                return False
            ...
            ws_logger.log(
                f"📤 Setting TP{target_num} for {symbol}: {tp_params}")

            if self.bot.use_demo_account:
                ws_logger.log(
                    f"🎮 DEMO: TP{target_num} {symbol} @ {target_price_str}")
                tp_response = {
                    'retCode': 0,
                ...
                ws_logger.log(
                    f"💰 LIVE: TP{target_num} {symbol} @ {target_price_str}")
            ...
            if tp_response.get('retCode') == 0:
                ...
                ws_logger.log(
                    f"✅ TP{target_num} set for {symbol}: Order ID {tp_order_id}",
                    'success')
                return True
            else:
                error_msg = tp_response.get('retMsg', 'Unknown error')
                ws_logger.log(
                    f"❌ Error TP{target_num} for {symbol}: {error_msg}", 'error')
                return False

        except Exception as e:
            ws_logger.log(
                f"❌ Error set_take_profit for {symbol} TP{target_num}: {e}",
                'error')
            return False
        ...
        try:
            ...
            if position_type == "long":
                if sl_price >= entry_price:
                    ...
                    ws_logger.log(error_msg, 'error')
                    ws_logger.log(
                        "💡 For LONG: Stop Loss must be BELOW entry price!", 'warning')
                    return False
            elif position_type == "short":
                if sl_price <= entry_price:
                    ...
                    ws_logger.log(error_msg, 'error')
                    ws_logger.log(
                        "💡 For SHORT: Stop Loss must be ABOVE entry price!", 'warning')
                    return False

            ws_logger.log(
                f"✅ SL validation PASSED for {
                    position_type.upper()} {symbol}",
                'success')
            ...
            ws_logger.log(
                f"🔄 Fallback: conditional Stop order for {symbol}",
                'warning')
            return self.set_conditional_stop(
                symbol, sl_price, current_position_size, position_type)
    ...
    def set_trading_stop(self, symbol, sl_price, position_type):
        ...
        try:
            ...
            ws_logger.log(
                f"📤 Set Trading Stop for {symbol}: {trading_stop_params}")

            if self.bot.use_demo_account:
                ws_logger.log(
                    f"🎮 DEMO: Trading Stop {symbol} @ {sl_price_str}")
                response = {
                    'retCode': 0,
                ...
                ws_logger.log(
                    f"💰 LIVE: Trading Stop {symbol} @ {sl_price_str}")
                response = self.bot.bybit_client.set_trading_stop(
                    **trading_stop_params)
            ...
            if response.get('retCode') == 0:
                ...
                ws_logger.log(
                    f"✅ Trading Stop set for {symbol} @ {sl_price_str}",
                    'success')
                return True
            else:
                error_msg = response.get('retMsg', 'Unknown error')
                ws_logger.log(
                    f"❌ Trading Stop error for {symbol}: {error_msg}", 'error')
                return False

        except Exception as e:
            ws_logger.log(
                f"❌ Error set_trading_stop for {symbol}: {e}",
                'error')
            return False

    def set_conditional_stop(self, symbol, sl_price,
                             position_size, position_type):
        ...
        try:
            ...
            if self.bot.use_demo_account:
                ws_logger.log(
                    f"🎮 DEMO: Conditional Stop {symbol} @ {sl_price_str}")
                response = {
                    'retCode': 0,
                ...
                ws_logger.log(
                    f"💰 LIVE: Conditional Stop {symbol} @ {sl_price_str}")
                response = self.bot.bybit_client.place_order(**stop_params)

            if response.get('retCode') == 0:
                sl_order_id = response.get(
                    'result', {}).get(
                    'orderId', 'CONDITIONAL_STOP')
                position = self.active_positions[symbol]
                position['sl_order_id'] = sl_order_id
                ws_logger.log(
                    f"✅ Conditional Stop set for {symbol}: {sl_order_id}",
                    'success')
                return True
            else:
                error_msg = response.get('retMsg', 'Unknown error')
                ws_logger.log(
                    f"❌ Conditional Stop error for {symbol}: {error_msg}", 'error')
                return False

        except Exception as e:
            ws_logger.log(
                f"❌ Error set_conditional_stop for {symbol}: {e}",
                'error')
            return False
    ...
    def cancel_sl_order(self, symbol, order_id):
        ...
        try:
            ...
            ws_logger.log(
                f"❌ Error cancel_sl_order for {symbol}: {e}",
                'error')
            return False
    ...
    def cancel_trading_stop(self, symbol):
        ...
        try:
            ...
            if response.get('retCode') == 0:
                ws_logger.log(
                    f"✅ Trading Stop removed for {symbol}",
                    'success')
                return True
            else:
                error_msg = response.get('retMsg', 'Unknown error')
                ws_logger.log(
                    f"❌ Error removing Trading Stop for {symbol}: {error_msg}", 'error')
                return False

        except Exception as e:
            ws_logger.log(
                f"❌ Error cancel_trading_stop for {symbol}: {e}",
                'error')
            return False
        ...
        try:
                ...
                if position:
                    ...
                    if self.bot.use_percentage:
                        ...
                        if balance_info and balance_info.get('success'):
                            ...
                            amount_to_use = available * \
                                (self.bot.default_amount / 100.0)
                    ...
                    symbol_info = self.bot.get_symbol_info(symbol)
                    if symbol_info:
                        qty_formatted = self.bot.format_quantity(
                            qty, symbol_info)
            ...
            try:
                ...
                if positions_response.get('retCode') == 0:
                    position_list = positions_response.get(
                        'result', {}).get('list', [])
                    ...
                    for pos in position_list:
                        if pos.get('symbol') == symbol:
                            ...
                            if size > 0:
                                ws_logger.log(
                                    f"📊 Current position {symbol}: {size}")
                                return size

                    ws_logger.log(
                        f"⚠️ No active position for {symbol}", 'warning')
                    return 0
                else:
                    error_msg = positions_response.get(
                        'retMsg', 'Unknown error')
                    ws_logger.log(
                        f"❌ Error getting position {symbol}: {error_msg}", 'error')
                    return 0

            except Exception as e:
                ws_logger.log(
                    f"❌ API error get_positions for {symbol}: {e}", 'error')
                return 0

        except Exception as e:
            ws_logger.log(
                f"❌ Error get_current_position_size for {symbol}: {e}",
                'error')
            return 0
        ...
        try:
            ...
            try:
                ...
                if ticker_response.get('retCode') == 0:
                    ticker_list = ticker_response.get(
                        'result', {}).get('list', [])
                ...
                ws_logger.log(
                    f"❌ API error get_tickers for {symbol}: {e}", 'error')
                return None

        except Exception as e:
            ws_logger.log(
                f"❌ Error get_current_price for {symbol}: {e}",
                'error')
            return None
    ...
    def check_filled_orders(self, symbol):
        ...
        try:
            ...
            if price_check_result:
                ws_logger.log(
                    f"✅ New targets detected by price check for {symbol}",
                    'success')
            ...
            for target_num, order_id in position['tp_orders'].items():
                if target_num not in position['filled_targets']:
                    if self.is_order_filled(symbol, order_id):
                        ...
                        ws_logger.log(
                            f"🎯 TP{target_num} reached for {symbol} (by order status)!", 'success')

                        # If it's trigger target, move SL to breakeven
                        breakeven_target = position.get('breakeven_target', 1)
                        if target_num == breakeven_target and not position[
                                'sl_moved_to_breakeven'] and self.bot.auto_breakeven:
                            ws_logger.log(
                                f"💡 Target {breakeven_target} by order - activating breakeven for {symbol}")
            ...
            ws_logger.log(
                f"❌ Error check_filled_orders for {symbol}: {e}",
                'error')
    ...
    def is_order_filled(self, symbol, order_id):
        ...
        try:
            ...
            try:
                    ...
                    for order in orders:
                        if order.get('orderId') == order_id:
                            ...
                            if status in ['Filled', 'PartiallyFilled']:
                                ws_logger.log(
                                    f"✅ Order {order_id} filled (status: {status})", 'success')
                                return True
                            else:
                                ws_logger.log(
                ...
                ws_logger.log(
                    f"❌ Error checking order status {order_id}: {e}", 'error')
                return False
    ...
    def simulate_order_fill_improved(self, symbol, order_id):
        ...
        try:
                ...
                if tp_order_id == order_id:
                    ...
                    if target_price:
                        ...
                        if position_type == "long" and current_price >= target_price:
                            ws_logger.log(
                                f"🎮 DEMO: TP{target_num} simulated fill @ {current_price}")
                            return True
            ...
            # Check if it's SL
            if position['sl_order_id'] == order_id and position.get(
                    'stop_loss'):
                ...
                if position_type == "long" and current_price <= sl_price:
                    ws_logger.log(
                        f"🎮 DEMO: SL simulated fill @ {current_price}")
                    return True
            ...
            ws_logger.log(
                f"❌ Error simulate_order_fill_improved: {e}",
                'error')
            return False
    ...
    def remove_position(self, symbol):
        ...
        try:
            if symbol in self.active_positions:
                ...
                ws_logger.log(
                    f"✅ Position {symbol} removed from monitoring",
                    'success')
                ...
                if active_symbols:
                    ...
                    for symbol in active_symbols:
                        try:
                            ...
                            if position:
                                filled_count = len(
                                    position.get('filled_targets', set()))
                                total_targets = len(
                                    position.get('targets', {}))
                                breakeven_status = "✅" if position.get(
                                    'sl_moved_to_breakeven') else "❌"
                                breakeven_trigger = position.get(
                                    'breakeven_target', 1)

                                ws_logger.log(
                                    f"📊 {symbol}: Targets {filled_count}/{total_targets}, Breakeven {breakeven_status} (trigger: TP{breakeven_trigger})")

                        except Exception as e:
                            ws_logger.log(
                                f"❌ Monitoring error {symbol}: {e}", 'error')
                            continue
                else:
...
class TelegramTradingBot:
    ...
    def __init__(self):
        ...
        self.telegram_chat_id = self.config.get(
            'Telegram', 'chat_id', fallback='')

        # Bybit API
        self.bybit_api_key = self.config.get('Bybit', 'api_key', fallback='')
        self.bybit_api_secret = self.config.get(
            'Bybit', 'api_secret', fallback='')
        self.bybit_subaccount = self.config.get(
            'Bybit', 'subaccount', fallback='')
        self.bybit_platform = self.config.get(
            'Bybit', 'platform', fallback='bybitglobal')
        self.position_mode = self.config.get(
            'Bybit', 'position_mode', fallback='one_way')
        ...
        self.default_leverage = int(
            self.config.get(
                ...
                'default_leverage',
                fallback='10'))
        self.default_amount = float(
            self.config.get(
                'Trading',
                'default_amount',
                fallback='100'))
        self.use_percentage = self.config.get(
            'Trading', 'use_percentage', fallback='False').lower() == 'true'
        self.use_demo_account = self.config.get(
            'Trading', 'use_demo_account', fallback='True').lower() == 'true'
        ...
        self.auto_tp_sl = self.config.get(
            'Trading', 'auto_tp_sl', fallback='True').lower() == 'true'
        self.auto_breakeven = self.config.get(
            'Trading', 'auto_breakeven', fallback='True').lower() == 'true'
        self.breakeven_after_target = int(
            self.config.get(
                'Trading',
        ...
        self.max_consecutive_losses = int(
            self.config.get(
                ...
                fallback='3'))
        self.risk_management_enabled = self.config.get(
            'RiskManagement', 'enabled', fallback='True').lower() == 'true'
        self.min_margin_level = float(
            self.config.get(
                'RiskManagement',
                'min_margin_level',
                fallback='1.5'))  # NEW
        ...
        self.auto_execute_signals = self.config.get(
            'Trading', 'auto_execute_signals', fallback='False').lower() == 'true'
    ...
    async def telegram_message_handler(
        ...
        try:
            ...
            if self.telegram_chat_id and chat_id != self.telegram_chat_id:
                ws_logger.log(
                    f"🚫 Message from unauthorized chat_id: {chat_id}",
                    'warning')
                return

            ws_logger.log(
                f"📨 Received message from Telegram (Chat ID: {chat_id})")
    ...
    def start_telegram_bot(self):
        ...
        try:
            if not self.telegram_token:
                ws_logger.log(
                    "⚠️ No Telegram token - bot won't start",
                    'warning')
                return False
    ...
    def get_telegram_chat_id(self):
        ...
        try:
            ...
            url = f"https://api.telegram.org/bot{
                self.telegram_token}/getUpdates"
    ...
    def save_config(self):
        ...
        try:
            ...
            self.config['Trading']['breakeven_after_target'] = str(
                self.breakeven_after_target)
            self.config['Trading']['auto_execute_signals'] = str(
                self.auto_execute_signals)
            self.config['Trading']['max_position_size'] = str(
            ...
            self.config['RiskManagement']['enabled'] = str(
                self.risk_management_enabled)
            self.config['RiskManagement']['daily_loss_limit'] = str(
                self.daily_loss_limit)
            self.config['RiskManagement']['weekly_loss_limit'] = str(
                self.weekly_loss_limit)
            self.config['RiskManagement']['max_consecutive_losses'] = str(
                self.max_consecutive_losses)
            self.config['RiskManagement']['min_margin_level'] = str(
    ...
    def save_current_as_profile(self, profile_name):
        ...
        try:
            ...
            return self.profile_manager.save_profile(
    ...
    def load_profile(self, profile_name):
        ...
        try:
            ...
            self.bybit_platform = profile_data.get(
                'bybit_platform', 'bybitglobal')
            ...
            self.breakeven_after_target = profile_data.get(
                'breakeven_after_target', 1)
            self.auto_execute_signals = profile_data.get(
                'auto_execute_signals', False)
            self.max_position_size = profile_data.get(
                'max_position_size', 1000)
            self.risk_percent = profile_data.get('risk_percent', 2)
            self.risk_management_enabled = profile_data.get(
                'risk_management_enabled', True)
            self.daily_loss_limit = profile_data.get('daily_loss_limit', 500)
            self.weekly_loss_limit = profile_data.get(
                'weekly_loss_limit', 2000)
            self.max_consecutive_losses = profile_data.get(
                'max_consecutive_losses', 3)
    ...
    def initialize_bybit_client(self):
        """Initialize Bybit API client - ENHANCED WITH BETTER ERROR HANDLING"""
        try:
            from pybit.unified_trading import HTTP
        except ImportError:
            logger.error("Pybit HTTP not available")
            self.bybit_client = None
            return False
        try:
            ...
            ws_logger.log(
                f"DEBUG: API secret exists: {
                    bool(
                        self.bybit_api_secret)}")
            ws_logger.log(f"DEBUG: Platform: {self.bybit_platform}")
            ...
            # Create HTTP client with correct parameters
            ws_logger.log("📡 Creating HTTP client...")

            try:
                # IMPORTANT: For testnet/demo MUST use testnet=True
                if self.use_demo_account:
                    ws_logger.log("🎮 Using Bybit Testnet (Demo)")
                    self.bybit_client = HTTP(
                        api_key=clean_api_key,
                        api_secret=clean_api_secret,
                        testnet=True  # ALWAYS True for demo/testnet keys!
                    )
                else:
                    ws_logger.log("💰 Using Bybit Mainnet (Live)")
                    self.bybit_client = HTTP(
                        api_key=clean_api_key,
                        api_secret=clean_api_secret,
                        testnet=False  # False for live trading
                    )
                ...
                if server_time.get('retCode') == 0:
                    ws_logger.log("✅ Connected to Bybit API", 'success')
                    ...
                    try:
                        ...
                        ws_logger.log(
                            f"DEBUG: Balance response code: {
                                balance_response.get('retCode')}")

                        if balance_response.get('retCode') == 0:
                            balance_list = balance_response.get(
                                'result', {}).get('list', [])
                            if balance_list:
                                account = balance_list[0]
                                available = float(account.get(
                                    'totalAvailableBalance', '0'))
                                ws_logger.log(
                                    f"💰 Balance available: {
                                        available:.2f} USDT", 'success')
                            else:
                                ws_logger.log(
                                    "⚠️ Balance data empty but connection OK", 'warning')
                        elif balance_response.get('retCode') == 10001:
                            # Common error for testnet accounts without balance
                            ws_logger.log(
                                "⚠️ Account not activated or no balance (normal for new testnet)", 'warning')
                        else:
                            ws_logger.log(
                                f"⚠️ Balance fetch failed: {
                                    balance_response.get('retMsg')}", 'warning')
                    except Exception as balance_error:
                        ws_logger.log(
                            f"⚠️ Balance check failed but connection works: {balance_error}", 'warning')

                    # Connection successful even if balance fails
                    return True
                else:
                    error_msg = server_time.get('retMsg', 'Unknown error')
                    ws_logger.log(f"❌ Connection failed: {error_msg}", 'error')
                    self.bybit_client = None
                    return False
    ...
    def get_subaccounts(self):
        ...
        try:
            ...
            if response.get('retCode') == 0:
                ...
                ws_logger.log(
                    f"✅ Fetched {
                        len(subaccounts)} subaccounts",
                    'success')
                return subaccounts
            else:
                ws_logger.log(
                    f"❌ Error fetching subaccounts: {
                        response.get(
                            'retMsg',
                            'Unknown error')}",
                    'error')
        ...
        try:
            if not self.bybit_client:
                ...
                if not self.initialize_bybit_client():
                    ...
                    if self.use_demo_account:
                        ws_logger.log(
                            "🎮 Returning simulated demo balance", 'info')
                        return {
                            'success': True,
            ...
            try:
                ...
                try:
                    ...
                    ws_logger.log(
                        f"DEBUG: UNIFIED balance response code: {
                            balance_response.get('retCode')}")

                except Exception as unified_error:
                    ws_logger.log(
                        f"⚠️ UNIFIED account error: {unified_error}", 'warning')
                    ...
                    try:
                        ...
                        ws_logger.log(
                            f"DEBUG: CONTRACT balance response code: {
                                balance_response.get('retCode')}")
                    except Exception as contract_error:
                        ws_logger.log(
                            f"⚠️ CONTRACT account error: {contract_error}", 'warning')
                ...
                if balance_response and balance_response.get('retCode') == 0:
                    ...
                    if balance_list:
                        ...
                        ws_logger.log(
                            f"✅ Balance fetched successfully", 'success')
                        ws_logger.log(
                            f"💵 Available: {
                                total_available_balance:.2f} USDT")
                        ws_logger.log(
                            f"💰 Wallet: {
                                total_wallet_balance:.2f} USDT")
                        ws_logger.log(
                            f"📊 Margin: {
                                total_margin_balance:.2f} USDT")
                ...
                # No valid balance response - return demo balance if in demo
                # mode
                if self.use_demo_account:
    ...
    def get_symbol_info(self, symbol):
        ...
        try:
            ...
            if instruments_response.get('retCode') == 0:
                instruments = instruments_response.get(
                    'result', {}).get('list', [])
                if instruments:
    ...
    def parse_trading_signal(self, text):
        ...
        try:
            ...
            ws_logger.log(
                f"📊 Symbol: {
                    symbol_match.group(1) if symbol_match else 'Not found'}")
            ws_logger.log(
                f"📈 Position: {
                    position_match.group(1) if position_match else 'Not found'}")
            ws_logger.log(
                f"💰 Entry: {
                    entry_match.group(1) if entry_match else 'Not found'}")
            ws_logger.log(f"🎯 Targets: {len(target_matches)} found")
            ws_logger.log(
                f"🛑 Stop Loss: {
                    sl_match.group(1) if sl_match else 'Not found'}")
            ...
            if '-' in entry_str:
                ...
                ws_logger.log(
                    f"📊 Entry range {prices[0]} - {prices[1]}, using average: {entry_price}")
            else:
        ...
        try:
            ...
            if signal['stop_loss']:
                ...
                if signal['position_type'] == 'long':
                    ...
                    for target_num, target_price in sorted(
                            signal['targets'].items()):
                        reward_percent = ((target_price - entry) / entry) * 100
                        rr_ratio = reward_percent / risk_percent if risk_percent > 0 else 0
                        analysis.append(
                            f"🎯 Target {target_num}: +{reward_percent:.2f}% (R:R = 1:{rr_ratio:.1f})")
                else:  # short
                ...
                for target_num, target_price in sorted(
                    ...
                    if signal['position_type'] == 'long':
                        reward_percent = ((target_price - entry) / entry) * 100
                        analysis.append(
                            f"🎯 Target {target_num}: +{reward_percent:.2f}%")
                    else:
                        reward_percent = ((entry - target_price) / entry) * 100
                        analysis.append(
                            f"🎯 Target {target_num}: +{reward_percent:.2f}%")
    ...
    def execute_trade(self, signal):
        ...
        try:
            ...
            if position_value_usdt > self.max_position_size:
                ...
                ws_logger.log(
                    f"⚠️ Limited to max position: {
                        self.max_position_size} USDT")
            ...
            ws_logger.log(
                f"   📈 Position value: {
                    position_value_usdt:.2f} USDT")
            ws_logger.log(f"   📊 Quantity: {qty_formatted} {symbol}")

            # Set leverage
            try:
                ws_logger.log(
                    f"⚔️ Setting leverage {
                        self.default_leverage}x...")
                ...
                if leverage_response.get('retCode') == 0:
                    ...
                    ws_logger.log(
                        f"⚠️ Error setting leverage: {
                            leverage_response.get(
                                'retMsg',
                                'Unknown error')}",
                        'warning')
            ...
            if order_response.get('retCode') == 0:
                order_id = order_response.get(
                    ...
                    'orderId', 'Unknown')
                ws_logger.log(
                    f"✅ Order executed! Order ID: {order_id}",
                    'success')
...
{'=' * 40}"""
                ...
                if self.auto_breakeven:
                    result += f"\n💡 Auto Breakeven: Active (after reaching TP{
                        self.breakeven_after_target})"
                ...
                if self.risk_management_enabled:
                    # Record small loss for failed trade
                    self.risk_manager.record_trade(
                        -amount_to_use * 0.01, symbol)
    ...
    async def _telethon_channels(self):
        ...
        try:
            ...
            for i, ch in enumerate(
                    channels[:20], 1):  # Limit to 20 for console
...
Cooling Until: {stats['cooling_until'] or 'None'}"""
    ...
    def _risk_margin(self):
        ...
        balance = self.bot.get_wallet_balance()
        if balance:
            is_safe, msg = self.bot.risk_manager.check_margin_level(
                balance, self.bot.min_margin_level)
...
def handle_config():
    ...
    try:
        if request.method == 'GET':
            ...
            try:
                ...
                if bot.save_config():
                    ...
                    return jsonify(
                        {'success': False, 'error': 'Failed to save configuration'}), 500
        ...
        return jsonify(
            {'success': False, 'error': f'Server error: {str(e)}'}), 500
...
def test_connection():
    ...
    try:
        if bot.initialize_bybit_client():
            ...
            return jsonify(
                {'success': False, 'error': 'Connection failed'}), 400
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def handle_profiles():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if not name:
                return jsonify(
                    {'success': False, 'error': 'Profile name required'}), 400

            if bot.save_current_as_profile(name):
                return jsonify(
                    {'success': True, 'message': f'Profile {name} saved'})
            else:
                return jsonify(
                    {'success': False, 'error': 'Failed to save profile'}), 500
...
def load_profile(name):
    ...
    try:
        if bot.load_profile(name):
            return jsonify(
                {'success': True, 'message': f'Profile {name} loaded'})
        else:
            return jsonify(
...
def delete_profile(name):
    ...
    try:
        if bot.profile_manager.delete_profile(name):
            return jsonify(
                {'success': True, 'message': f'Profile {name} deleted'})
        else:
            return jsonify(
                {'success': False, 'error': 'Profile not found'}), 404
    except Exception as e:
...
def update_trading_settings():
    ...
    try:
        ...
        if bot.save_config():
            return jsonify(
                {'success': True, 'message': 'Trading settings saved'})
        else:
            return jsonify(
                {'success': False, 'error': 'Failed to save settings'}), 500
...
def get_balance():
    ...
    try:
        ...
        if balance:
            ...
            return jsonify(
                {'success': False, 'error': 'Failed to get balance'}), 400
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def analyze_signal():
    ...
    try:
        ...
        if signal:
            ...
            return jsonify(
                {'success': False, 'error': 'Signal not recognized'}), 400
...
def execute_trade():
    ...
    try:
        ...
        if not signal:
            return jsonify(
                {'success': False, 'error': 'No signal provided'}), 400
...
def start_telegram():
    ...
    try:
        ...
        if 'auto_execute' in data:
            ...
            ws_logger.log(
                f"🤖 Auto-trade set to: {bot.auto_execute_signals}", 'info')
        ...
        if bot.start_telegram_bot():
            return jsonify(
                {'success': True, 'message': 'Telegram bot started'})
        else:
            return jsonify(
                {'success': False, 'error': 'Failed to start Telegram bot'}), 500
    except Exception as e:
...
def stop_telegram():
    ...
    try:
        if bot.stop_telegram_bot():
            return jsonify(
                {'success': True, 'message': 'Telegram bot stopped'})
        else:
            return jsonify(
                {'success': False, 'error': 'Failed to stop bot'}), 400
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_chat_id():
    ...
    try:
        ...
        if chat_id:
            ...
            return jsonify(
                {'success': False, 'error': 'No chat ID found'}), 404
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def update_forwarder_config():
    ...
    try:
        ...
        bot.forwarder.forward_all_messages = data.get(
            'forward_all_messages', False)

        # Save configuration
        if bot.forwarder.save_forwarder_config():
            ws_logger.log(
                "✅ Forwarder configuration saved successfully",
                'success')
            return jsonify(
                {'success': True, 'message': 'Forwarder config saved'})
        else:
            ws_logger.log("❌ Error saving forwarder configuration", 'error')
            return jsonify(
                {'success': False, 'error': 'Failed to save config'}), 500
...
def get_forwarder_channels():
    ...
    try:
        ...
        return jsonify(
            {'success': False, 'error': f'Error fetching channels: {str(e)}'}), 500
...
def add_channel_to_monitoring():
    ...
    try:
        ...
        if not channel_id or not channel_name:
            return jsonify(
                {'success': False, 'error': 'Channel ID and name required'}), 400
        ...
        existing = [
            ch for ch in bot.forwarder.monitored_channels if ch['id'] == channel_id]
        if existing:
            return jsonify(
                {'success': False, 'error': 'Channel already monitored'}), 400
    ...
    try:
        if 0 <= index < len(bot.forwarder.monitored_channels):
            ...
            if bot.forwarder.save_forwarder_config():
                ...
                return jsonify(
                    {'success': False, 'error': 'Failed to save'}), 500
        else:
...
def start_forwarder():
    ...
    try:
        ...
        if bot.forwarder.start_forwarder():
            ...
            return jsonify(
                {'success': False, 'error': 'Failed to start forwarder. Check logs.'})
    except Exception as e:
...
def stop_forwarder():
    ...
    try:
        if bot.forwarder.stop_forwarder():
            ...
            return jsonify(
                {'success': False, 'error': 'Failed to stop forwarder'}), 400
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def set_breakeven():
    ...
    try:
        ...
        if bot.position_manager.move_sl_to_breakeven(symbol):
            return jsonify(
                {'success': True, 'message': f'Breakeven set for {symbol}'})
        else:
            return jsonify(
                {'success': False, 'error': 'Failed to set breakeven'}), 400
...
def remove_position(symbol):
    ...
    try:
        ...
        return jsonify(
            {'success': True, 'message': f'Position {symbol} removed'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
